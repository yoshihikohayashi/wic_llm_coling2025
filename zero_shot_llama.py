##### for llama3-8B 2024-07-02 09:33:03 #####
...
zero_shot_template__ = '''The task is to identify if the meanings of the target word \"{word}\" in the following c1 and c2 sentences correspond to {adj} meanings or not.
Target word: {word}
c1: {c1}
c2: {c2}
If the meanings correspond to {adj} meanings, Answer T. Otherwise, Answer F.
...
def main():
    ...
    arg_p.add_argument('--llm', type=str, default='llama3-8B', help='llama model')
    ...
    if args.verbose=='True':
        ...
        args.verbose = False
    #
    pred_list, golds, target_df = wic_llama(pipe=pipe_gen, adj=args.adj, 
...
def wic_llama(pipe=pipe_gen, adj='same', 
              llm='llama-3-8B', run_id=23,
              target_ds='dev', 
              target_df='',
              s=0, e=20,
    ...
    #
    pred_list = parse_llama_results(completions, pr_template.name)
    if verbose:
        print('Accuracy:', comp_acc(pred_list, golds))
    return pred_list, golds, target_df

def parse_llama_results(completions, pr_name, anti=False):
...
def test(s, e=0, pipe=pipe_gen, adj='same', ds='dev', pr=zero_shot_prompt, max_tokens=10):
    ds = make_df(ds)
    if e==0: e = len(ds)
    X = wic_llama(pipe, adj, ds, s=s, e=e, pr_template=pr, max_tokens=max_tokens)
